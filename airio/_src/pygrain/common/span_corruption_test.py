...
class _MapWithPresetSeeds(grain.MapDataset):
...
def mock_random_map_fn_lazy_map_dataset(ds, map_fn, seed):
  ...
  ds = grain.MapDataset.source(list(iter(ds)))
...
class SpanCorruptionTest(absltest.TestCase):
  ...
  def test_span_corruption(self):
    ...
    airio_ds = grain.MapDataset.source(examples)
...
class BatchedDenoiseTest(absltest.TestCase):
  ...
  def test_denoise_batched(self):
    ...
    examples = list(src_ds.as_numpy_iterator())
    ds = grain.MapDataset.source(examples)
    runtime_args = test_utils.create_airio_injected_runtime_args(
